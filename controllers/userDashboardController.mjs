...
const getUser = (cookie) => {
  jwt.verify(cookie, "my secret code goes here", async (err, token) => {
    if (token) {
      const user = await userModel.User.findById(token.id);
      if (user) {
...
const user_dashboard_get = async (req, res) => {
  // requests = await requestModel.userRequests(req.user);
...
const user_send_request_post = async (req, res) => {
  ...
  if (
    await requestModel.checkRequestValidity(
      newRequest.userID,
      newRequest.bookID
    )
  ) {
